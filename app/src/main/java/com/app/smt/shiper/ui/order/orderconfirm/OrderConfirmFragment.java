...
import com.app.smt.shiper.ui.order.orderconfirm.adapter.OrderConfirmAdapter;
...
public class OrderConfirmFragment extends BaseFragment implements OrderConfirmMvpView, OrderConfirmAdapter.OrderAdapterListener,
        StoreOptionAdapter.StoreOptionAdapterListener, SwipeRefreshLayout.OnRefreshListener {

    public static final String TAG = OrderConfirmFragment.class.getSimpleName();
    ...
    protected void setUp(View view) {
        ...
        mAdapter = new OrderConfirmAdapter(getBaseActivity(), this);
    ...
    public void onMessageEvent(String message) {
        if (AppConstants.EVENTBUS_LIST_ORDER_WAITING_CONFIRM.equals(message)) {
            fetchData(1, true);
        }
    }
    ...
    @Override
    public void onConfirmOrderSuccess(int index, Order order) {
        mPresenter.callApiChangeWorkflow(index, order.getTxtId());
    }

    @Override
    public void onConfirmOrderFail(final int index, final Order order) {
        if (AppConstants.CHANGE_MONEY.equals(order.getSaleOrderInformationCommon().getChangeType())) {
            mPresenter.callApiRejectChangeMoney(index, order.getTxtId());
        } else {
            mPresenter.calApiRejectOrderForOtherShipper(index, order.getTxtId());
        }
